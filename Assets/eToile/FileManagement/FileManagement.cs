...
public static class FileManagement
{
    ...
    public static void SaveRawFile(string name, byte[] content, bool enc = false, bool fullPath = false)
    {
        ...
        {
            if (!fullPath)
                name = Combine(Application.persistentDataPath, name);
            if (content != null)
            {
    ...
    public static AudioClip ImportAudio(string file, bool enc = false, bool checkSA = true, bool fullPath = false)
    {
        if (!fullPath)
        {
            ...
            if (FileExists(file, false))
                ...
                file = Combine(Application.streamingAssetsPath, file);
        }
    ...
    private static bool CheckNameOnIndex(string name, string type)
    {
        ...
        if(blocks == null)
        {
            ...
            string indexPath = Application.streamingAssetsPath + "/FMSA_Index";
            if (Application.platform == RuntimePlatform.WebGLPlayer)
            {
    ...
    public static AudioClip ImportAudio(string file, bool enc = false, bool checkSA = true, bool fullPath = false)
    {
        ...
        if(fileExt == ".wav")
        {
             ...
             * 
             */
            byte[] wavFile = ReadRawFile(file);
            //int _chunkSize = System.BitConverter.ToInt32(wavFile, 4);
            ...
            //int _byteRate = System.BitConverter.ToInt32(wavFile, 28);
            //int _blockAlign = System.BitConverter.ToInt16(wavFile, 32);
            int _bitsPerSample = System.BitConverter.ToInt16(wavFile, 34);  // Not used?
                                                                            // Data parameters:
            int _subchunk2Size = System.BitConverter.ToInt32(wavFile, 20 + _subchunk1Size + 4); // Data size.
            int _dataIndex = 20 + _subchunk1Size + 8;                                           // Where data starts.
    ...
    private static bool CheckNameOnIndex(string name, string type)
    {
        ...
        {
            // Load blocks first time:
            string indexPath = Combine(Application.streamingAssetsPath, "FMSA_Index");
            byte[] contentArray = { };
    ...
    public static bool FileExists(string name, bool checkSA = true, bool fullPath = false)
    {
        ...
        if (!fullPath)
        {
            ...
            string path = Combine(Application.persistentDataPath, name);
    ...
    public static bool DirectoryExists(string folder = "", bool checkSA = true, bool fullPath = false)
    {
        ...
        {
            // Check PersistentData path first:
            string path = Combine(Application.persistentDataPath, folder);
    ...
    public static void CopyDirectory(string source, string dest, bool checkSA = true, bool fullPathSource = false, bool fullPathDest = false)
    {
        if (source != "" && dest != "")
        {
            ...
            if (DirectoryExists(source, checkSA, fullPathSource)) // If source exists on persistent data, full path or StreamingAssets.
            {
                ...
                {
                    string pathS = Combine(source, files[f]);   // Source file path
                    string pathD = Combine(dest, files[f]);     // Destination file path
                    CopyFile(pathS, pathD, checkSA, fullPathSource, fullPathDest);
                }
                // Iterate on every folder:
                for(int f = 0; f < folders.Length; f++)
                {
                    string pathS = Combine(source, folders[f]); // Source folder path.
                    string pathD = Combine(dest, folders[f]);   // Destination folder path.
    ...
    public static void Move(string source, string dest, bool fullPathSource = false, bool fullPathDest = false)
    {
        if (source != "" && dest != "")
        {
            ...
            {
                if (!fullPathSource)
                    source = Combine(Application.persistentDataPath, source);
                if (!fullPathDest)
                    dest = Combine(Application.persistentDataPath, dest);
