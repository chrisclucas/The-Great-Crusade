...
namespace TheGreatCrusade
{
    ...
    public class CreateBoard : MonoBehaviour
    {
        ...
        private void ReadHexFile()
        {
            ...
            if (!File.Exists(GlobalGameFields.hexSetupFile))
            {
                // There is no recovering from this error but let the user know
                GlobalDefinitions.GuiUpdateStatusMessage("Internal Error - Unable to open the hex file " + GlobalGameFields.hexSetupFile);
                return;
            }
            theReader = new StreamReader(GlobalGameFields.hexSetupFile);
            lineNumber = 0;
            using (theReader)
            {
                do
            ...
            hex = GeneralHexRoutines.GetHexAtXY(8, 23);
            {
            ...
            hex = GeneralHexRoutines.GetHexAtXY(14, 16);
            {
                targetRenderer = hex.GetComponent(typeof(SpriteRenderer)) as Renderer;
                //hex.transform.localScale = new Vector2(0.75f, 0.75f);
                targetRenderer.sortingLayerName = "Hex";
            ...
            hex = GeneralHexRoutines.GetHexAtXY(22, 1);
            {
                targetRenderer = hex.GetComponent(typeof(SpriteRenderer)) as Renderer;
                //hex.transform.localScale = new Vector2(0.75f, 0.75f);
                targetRenderer.sortingLayerName = "Hex";
        ...
        {
            // Check that the hex file exists
            if (!File.Exists(GlobalGameFields.riverSetupFile))
            {
                // There is no recovering from this error but let the user know
                GlobalDefinitions.GuiUpdateStatusMessage("Internal Error - Unable to open the hex file " + GlobalGameFields.riverSetupFile);
                return;
            }
            theReader = new StreamReader(GlobalGameFields.riverSetupFile);
            lineNumber = 0;
            using (theReader)
                do
                {
                        ...
                        switch (switchEntries[0])
                        {
                            case "River":
                                ...
                                while (line != "}")
                                {
                                    ...
                                    {
                                        UpdateRiverZOCBetweenHexes(GeneralHexRoutines.GetHexAtXY(
                                                Convert.ToInt32(entries[riverX1Word]),
                                                Convert.ToInt32(entries[riverY1Word])),
                                                GeneralHexRoutines.GetHexAtXY(
        ...
        {
            // Check that the hex file exists
            if (!File.Exists(GlobalGameFields.hexSettingsFile))
            {
                // There is no recovering from this error but let the user know
                GlobalDefinitions.GuiUpdateStatusMessage("Internal Error - Unable to open the hex settings file " + GlobalGameFields.hexSettingsFile);
                return;
            }
            theReader = new StreamReader(GlobalGameFields.hexSettingsFile);
            lineNumber = 0;
            using (theReader)
            {
                do
                    ...
                    if (line != null)
                    {
                        ...
                        switch (switchEntries[0])
                        {
                            case "InlandPort":
                                ...
                                hexInstance = GeneralHexRoutines.GetHexAtXY(Convert.ToInt32(switchEntries[inlandPortX]), Convert.ToInt32(switchEntries[inlandPortY]));
                                line = theReader.ReadLine();
                                lineNumber++;
                                while (line != "}")
                                {
                                    string[] entries = line.Split(delimiterChars);
                                    hexInstance.GetComponent<HexDatabaseFields>().controlHexes.Add(GeneralHexRoutines.GetHexAtXY(Convert.ToInt32(entries[hexXPositionWord]), Convert.ToInt32(entries[hexYPositionWord])));
                                    line = theReader.ReadLine();
                                    lineNumber++;
                                }
                                break;
                            case "FreeFrench":
                                ...
                                {
                                    string[] entries = line.Split(delimiterChars);
                                    GeneralHexRoutines.GetHexAtXY(Convert.ToInt32(entries[hexXPositionWord]), Convert.ToInt32(entries[hexYPositionWord])).GetComponent<HexDatabaseFields>().FreeFrenchAvailableHex = true;
                                    line = theReader.ReadLine();
                                    lineNumber++;
                                }
                                break;
                            case "AlliedVictory":
                                ...
                                {
                                    string[] entries = line.Split(delimiterChars);
                                    GeneralHexRoutines.GetHexAtXY(Convert.ToInt32(entries[hexXPositionWord]), Convert.ToInt32(entries[hexYPositionWord])).GetComponent<HexDatabaseFields>().AlliedVictoryHex = true;
                                    line = theReader.ReadLine();
                                    lineNumber++;
                                }
                                break;
                            case "HistoricalProgress":
                                ...
                                {
                                    string[] entries = line.Split(delimiterChars);
                                    GeneralHexRoutines.GetHexAtXY(Convert.ToInt32(entries[hexXPositionWord]), Convert.ToInt32(entries[hexYPositionWord])).GetComponent<HexDatabaseFields>().historyWeekCaptured = Convert.ToInt32(entries[historicalWeekCaptured]);
                                    line = theReader.ReadLine();
                                    lineNumber++;
                                }
                                break;
        ...
        private void ReadMapGraphics()
        {
            ...
            if (!File.Exists(GlobalGameFields.mapGraphicsFile))
            {
                // There is no recovering from this error but let the user know
                GlobalDefinitions.GuiUpdateStatusMessage("Internal Error - Unable to open the graphics file " + GlobalGameFields.mapGraphicsFile);
                return;
            }
            theReader = new StreamReader(GlobalGameFields.mapGraphicsFile);
            lineNumber = 0;
            using (theReader)
            {
                do
                        ...
                        switch (switchEntries[0])
                        {
                            ...
                            case "InvasionBoundaries":
                                ...
                                while (line != "}")
                                {
                                    ...
                                    DrawLineBetweenHexes(GeneralHexRoutines.GetHexAtXY(int.Parse(entries[0]), int.Parse(entries[1])), GeneralHexRoutines.GetHexAtXY(int.Parse(entries[2]), int.Parse(entries[3])), Color.red);
                ...
                foreach (GlobalDefinitions.HexSides hexSides in Enum.GetValues(typeof(GlobalDefinitions.HexSides)))
                {
                    ...
                    if (neighborHex != null)
                    {
                        ...
                        if (ZOCExtendToHex(hex.GetComponent<HexDatabaseFields>().Neighbors[(int)hexSides]))
                            hex.GetComponent<BooleanArrayData>().exertsZOC[(int)hexSides] = true;
                        else
                ...
                if (hex.GetComponent<HexDatabaseFields>().sea)
                {
                    foreach (GlobalDefinitions.HexSides hexSides in Enum.GetValues(typeof(GlobalDefinitions.HexSides)))
                    {
                        ...
                        if (hex.GetComponent<HexDatabaseFields>().invasionTarget == hex.GetComponent<HexDatabaseFields>().Neighbors[(int)hexSides])
                        {
                            ...
                            {
                                hex.GetComponent<BooleanArrayData>().exertsZOC[(int)hexSides] = true;
                                hex.GetComponent<HexDatabaseFields>().invasionTarget.GetComponent<BooleanArrayData>().exertsZOC[GlobalDefinitions.ReturnHexSideOpposide((int)hexSides)] = true;
                            }
                        }
                    }
                }
                ...
                using (theReader)
                {
                    do
                    {
                        ...
                        if (line != null)
                        {
                            ...
                            string[] lineEntries = line.Split(delimiterChars);
                            placedUnit = GameObject.Find(lineEntries[unitName]);
                            hex = GeneralHexRoutines.GetHexAtXY(Convert.ToInt32(lineEntries[xHexCoor]), Convert.ToInt32(lineEntries[yHexCoor]));
                            GeneralHexRoutines.PutUnitOnHex(placedUnit, hex);
        ...
        public void SetupInvasionAreas()
        {
            ...
            GlobalDefinitions.invasionAreas[0].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(44, 31));
            GlobalDefinitions.invasionAreas[0].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(45, 30));
            GlobalDefinitions.invasionAreas[0].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(45, 29));
            GlobalDefinitions.invasionAreas[0].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(46, 29));
            GlobalDefinitions.invasionAreas[0].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(47, 28));
            GlobalDefinitions.invasionAreas[0].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(47, 27));
            GlobalDefinitions.invasionAreas[0].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(47, 25));
            GlobalDefinitions.invasionAreas[0].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(47, 24));
            GlobalDefinitions.invasionAreas[0].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(46, 24));
            GlobalDefinitions.invasionAreas[0].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(46, 23));
            GlobalDefinitions.invasionAreas[0].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(46, 20));
            ...
            GlobalDefinitions.invasionAreas[1].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(37, 8));
            GlobalDefinitions.invasionAreas[1].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(35, 7));
            GlobalDefinitions.invasionAreas[1].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(34, 7));
            GlobalDefinitions.invasionAreas[1].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(32, 7));
            GlobalDefinitions.invasionAreas[1].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(32, 6));
            GlobalDefinitions.invasionAreas[1].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(31, 5));
            GlobalDefinitions.invasionAreas[1].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(29, 5));
            GlobalDefinitions.invasionAreas[1].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(27, 4));
            ...
            GlobalDefinitions.invasionAreas[2].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(25, 2));
            GlobalDefinitions.invasionAreas[2].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(22, 0));
            GlobalDefinitions.invasionAreas[2].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(20, 2));
            GlobalDefinitions.invasionAreas[2].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(20, 3));
            GlobalDefinitions.invasionAreas[2].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(20, 4));
            GlobalDefinitions.invasionAreas[2].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(21, 4));
            GlobalDefinitions.invasionAreas[2].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(21, 5));
            GlobalDefinitions.invasionAreas[2].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(21, 6));
            GlobalDefinitions.invasionAreas[2].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(21, 7));
            ...
            GlobalDefinitions.invasionAreas[3].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(19, 6));
            GlobalDefinitions.invasionAreas[3].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(16, 8));
            GlobalDefinitions.invasionAreas[3].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(16, 9));
            GlobalDefinitions.invasionAreas[3].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(18, 9));
            GlobalDefinitions.invasionAreas[3].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(18, 10));
            GlobalDefinitions.invasionAreas[3].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(18, 11));
            ...
            GlobalDefinitions.invasionAreas[4].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(17, 11));
            GlobalDefinitions.invasionAreas[4].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(17, 12));
            GlobalDefinitions.invasionAreas[4].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(16, 13));
            ...
            GlobalDefinitions.invasionAreas[5].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(16, 14));
            GlobalDefinitions.invasionAreas[5].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(15, 14));
            GlobalDefinitions.invasionAreas[5].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(14, 15));
            GlobalDefinitions.invasionAreas[5].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(13, 15));
            GlobalDefinitions.invasionAreas[5].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(12, 16));
            GlobalDefinitions.invasionAreas[5].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(12, 17));
            GlobalDefinitions.invasionAreas[5].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(11, 17));
            GlobalDefinitions.invasionAreas[5].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(11, 18));
            ...
            GlobalDefinitions.invasionAreas[6].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(10, 19));
            GlobalDefinitions.invasionAreas[6].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(9, 20));
            GlobalDefinitions.invasionAreas[6].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(8, 21));
            GlobalDefinitions.invasionAreas[6].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(7, 21));
            GlobalDefinitions.invasionAreas[6].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(6, 22));
            GlobalDefinitions.invasionAreas[6].invasionHexes.Add(GeneralHexRoutines.GetHexAtXY(5, 22));
        }
    }
}