...
public class FileTransferServer : MonoBehaviour
{
        ...
        if (Network.player.ipAddress != remoteIp)
        {
            switch (fields[0])
            {
                case "F0":  // Someone wants to know if this server is active.
                    ...
                    SendString(remoteIp, "F1;" + Network.player.ipAddress + ";" + enableServer.ToString() + ";#");
                    break;
                ...
                case "F4":  // Receiving a requested partial file.
                    ...
                    if (downloadList.Count > 0 && fields[1] == downloadList[0].serverIP && fields[2] == downloadList[0].file)
                    {
                        ...
                        if (completeCnt == partialFileList.Length)
                        {
                            ...
                            NetworkRoutines.SendMessageToRemoteComputer(GlobalDefinitions.GAMEDATALOADEDKEYWORD);
    ...
    public void CheckServerStatus(string serverIP)
    {
        ...
        // Send the response:
        SendString(serverIP, "F0;" + Network.player.ipAddress + ";#");
    }
    ...
    {
        SendString(serverIP, "F6;" + Network.player.ipAddress + ";" + file + ";#");
    ...
    void SendPartialFile(string ip, string name, string part = "1")
    {
        ...
        string tempCmd = "F4;" + Network.player.ipAddress + ";" + name + ";" + part + "," + parts.ToString() + ";";
    ...
    void SendUpdateList(string ip, string file)
    {
        ...
        if (FileManagement.FileExists(file, true))
        {
            ...
            string cmd = "F7;" + Network.player.ipAddress + ";";
    ...
    public void SendFile(string ip, string name)
    {
        if (enableServer)
        {
            if (FileManagement.FileExists(name))
            {
                string cmd = "F8;" + Network.player.ipAddress + ";" + name + ";#";    // F8;ServerIP;name;#
                SendString(ip, cmd);
            }
            else
            {
    ...
    public void SendUpdate(string ip, string list)
    {
        if (enableServer)
        {
            ...
            {
                string cmd = "F9;" + Network.player.ipAddress + ";" + list + ";#";    // F9;ServerIP;list;#
                SendString(ip, cmd);
            }
            else
            {
